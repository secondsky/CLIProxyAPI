...
func (s *FileSynthesizer) Synthesize(ctx *SynthesisContext) ([]*coreauth.Auth, error) {
	...
	// Preferred path: use multi-account aware loader + active-accounts.json
	...
	if home, errHome := os.UserHomeDir(); errHome == nil {
		defaultAuthDir := filepath.Join(home, accounts.DefaultDir)
		if samePath(ctx.AuthDir, defaultAuthDir) {
			providerAccounts, _ = accounts.LoadAllAccounts()
			activeAccounts = accounts.LoadActiveAccounts()
		}
	}
	now := ctx.Now
	cfg := ctx.Config
	...
	if len(providerAccounts) > 0 {
		for provider, list := range providerAccounts {
			chosen := accounts.ResolveActiveAccount(provider, list, activeAccounts)
			if chosen == nil {
				continue
			}
			...
			metadata := chosen.Raw
			if metadata == nil {
				// Fallback: read raw file if somehow missing
				full := filepath.Join(ctx.AuthDir, chosen.FileBase+".json")
				if data, errRead := os.ReadFile(full); errRead == nil && len(data) > 0 {
			...
			fullPath := filepath.Join(ctx.AuthDir, chosen.FileBase+".json")
			id := fullPath
	...
	for _, e := range entries {
		...
		if _, already := seenPaths[full]; already {
			continue
		}
